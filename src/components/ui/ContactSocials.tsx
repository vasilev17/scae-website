...
// Lucide dropped brand marks in v1, so the two wordless logos are inlined.
// Paths from simple-icons (CC0 1.0).
...
export function ContactSocials({ title, label, links }: ContactSocialsProps) {
  return (
    <section className="contact-socials" aria-label={label}>
      <h3 className="contact-socials-title">{title}</h3>
      <div className="contact-socials-array">
        <span className="contact-socials-bus" aria-hidden="true">
          <span className="contact-socials-pulse" />
        </span>
        <ul className="contact-socials-list">
          {links.map((link) => {
            ...
            return (
              <li key={link.icon}>
                <a
                  className="contact-socials-node"
                  href={link.href}
                  target={external ? '_blank' : undefined}
                  rel={external ? 'noreferrer' : undefined}
                >
                  <span className="contact-socials-diamond">
                    <Icon className="contact-socials-icon" />
                  </span>
                  <span className="contact-socials-label">{link.label}</span>
                </a>
              </li>
            );
          })}
